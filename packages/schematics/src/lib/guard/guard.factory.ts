...
export function main(options: GuardOptions) {
  ...
  return chain([
    ...
    move(
      join(path as Path, dir, base + '.guard.ts'),
      join(path as Path, dir, 'guards', base + '.guard.ts'),
    ),
    move(
      join(path as Path, dir, base + '.guard.spec.ts'),
      join(path as Path, dir, 'guards', base + '.guard.spec.ts'),
    ),
  ]);
}
